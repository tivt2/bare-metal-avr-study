...
- ### 8_i2c
  In this example we will work with another communication interface, the I2C interface. In summary the I2C is a communication protocol used for data exchange between two or more integrated circuits, utilizing two bidirectional bus lines to achieve communication between two or more integrated circuits. In the ATmega328P this interface can be found under [2-wire Serial Interface](https://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-7810-Automotive-Microcontrollers-ATmega328P_Datasheet.pdf#page=173).

  Even though the I2C is simple because it uses only two wires, its operation can be complex. Before starting the example we need to have another component capable to communicate via I2C, im using an accelerometer and gyroscope module for this example, the [MPU6050](https://components101.com/sensors/mpu6050-module). Some important documentation of the MPU6050 are [Product Specification](https://components101.com/sites/default/files/component_datasheet/MPU6050-DataSheet.pdf) and [Register Map and Descriptions](https://d229kd5ey79jzj.cloudfront.net/974/MPU-6000-Register-Map1.pdf).

  All devices utilizing the I2C have an identifiable address fixed by the manufacturer of the device. Each I2C device address is a 7-bit value, with the 8th bit reserved for indicating whether the operation is a read or write. For the MPU6050 the address can be found on its [data sheet](https://components101.com/sites/default/files/component_datasheet/MPU6050-DataSheet.pdf#page=15), being the address **0b1101000** in binary or **0x68** in hex.

  The 2 bus lines utilized by the I2C interface are the clock line (SCL) and the data line (SDA). The I2C defines four key roles for the devices communicating through the lines, **MASTER** is the device that initiates the I2C communication and is the source of the SCL, **SLAVE** is the device addressed by the MASTER, **TRANSMITTER** is the device writing data to the SDA bus line and **RECEIVER** is the device reading from the SDA line.

  When the ATmega328P operates as the master, it uses the [Bit Rate Generator Unit](https://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-7810-Automotive-Microcontrollers-ATmega328P_Datasheet.pdf#page=180) in order to control the SCL clock. Similar to USART, the SCL clock is configured by writing a calculated value to the TWBR register to produce a prescaled clock. This clock must match the requirements of the components the ATmega328P is interacting with. The MPU6050 module MAX SCL clock can be found [here](https://components101.com/sites/default/files/component_datasheet/MPU6050-DataSheet.pdf#page=15) under parameter I2C Operating Frequency, being 100kHz for standard-mode and 400kHz for fast-mode.

  The main idea of the I2C communication model is, when there is no communication through the lines, both the SCL and SDA lines are held HIGH by all the components connected to the lines. To start a communication, one component sends a START condition by pulling the SDA line LOW while SCL is maintained as HIGH, this will make that component a MASTER during the communication, the START condition signals to all other components that the lines are busy. After the START condition, all the components are listening the lines waiting for a SLA+W or SLA+R transmission sent by the MASTER, this will be an 8-bit value where the 7 most significant bits are the component address and the least significant bit indicates if its a W (write) or R (read) communication. The targeted component will then respond with an ACK signal making it the SLAVE in the communication. In write mode (SLA+W), the MASTER acts as a transmitter and sends data over the SDA line, while the slave responds with ACK or NACK. In read mode (SLA+R), the MASTER acts as a receiver and the SLAVE sends data over the SDA line, while the MASTER responds with ACK when there is more to be read or NACK when that was the last data read. A REPEAT START condition can be sent by the MASTER to change the mode (from SLA+W to SLA+R or vice versa) without releasing the lines, this prevents another component from taking control of the I2C lines. Once the communication is complete, the MASTER sends a STOP condition, which will return the lines to the idle state and pull both of them HIGH again.

  As a complement we can refer to an example reference under [Using the TWI](https://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-7810-Automotive-Microcontrollers-ATmega328P_Datasheet.pdf#page=181) where it describes step by step a simple interaction using I2C.

  A more explicit explanation of the I2C working behavior can be found at the ATmega328P documentation under [Data Transfer and Frame Format](https://ww1.microchip.com/downloads/en/DeviceDoc/Atmel-7810-Automotive-Microcontrollers-ATmega328P_Datasheet.pdf#page=174).

  For this example we will first initialize serial communication with our computer using [USART](###7_usart) and initialize a I2C communication with the MPU6050 accelerometer and gyroscope module and change its power configuration. After initialization we will constantly read the gyroscope values of from the MPU6050 and send it to our computer via USART so it can be displayed and we can visually see changes when we interact with the MPU6050.
